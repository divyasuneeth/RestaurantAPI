...
bing_api_key = 'Al2XWczV8lYKmPFGMtBKzNBl_5bsKo_a_Gi3RjrY7oFBx5Z0Tm1IEwAne6gPqaXd'
...
def getGeocodeLocation(inputString):
    ...
    url = ('https://dev.virtualearth.net/REST/v1/Locations?query=%s&key=%s'%(locationString, bing_api_key))
    #print url
    h= httplib2.Http()
    result = json.loads(h.request(url,'GET')[1])
    ##geocode = requests.get('https://maps.googleapis.com/maps/api/geocode/json?address="%s"&key=%s' % (locationString, google_api_key))
    ##geocode = json.loads(geocode.text)
    #print response
    ##latlan_dict=geocode['results'][0]['geometry']['location']
    ##latitude=latlng_dict['lat']
    ##longitude=latlng_dict['lng']
